...
export function ProductTile({ product }: { product: ProductCard }) {
  ...
  const soldOut = !product.variants.some((row) => row.available > 0);
  return (
    <article className="relative">
      <Link href={`/product/${product.slug}`} className="no-underline">
        <div className="relative aspect-[3/4] overflow-hidden rounded-2xl bg-ink/5">
          ...
          {soldOut ? (
            <span className="absolute left-3 top-3 rounded-full bg-primary/90 px-3 py-1 text-xs text-cream">Sold out</span>
          ) : null}
        </div>
        <p className="mt-3 font-medium">{product.title}</p>
        <p className="text-sm text-ink/70">{soldOut ? 'Sold out' : `${catalogPriceLabel(product.variants)}${priceTaxSuffix()}`}</p>
      </Link>
