export default function TermsPage() {
  return (
    <article className="max-w-2xl space-y-4">
      <h1 className="font-serif text-4xl">Terms</h1>
      <p>
        These terms apply to purchases from Motive Fashion, Dublin, Ireland, on the website, app, WhatsApp, and in-shop
        till. Distance contracts follow Irish and EU consumer law. Prices are in euro and include VAT.
      </p>
      <p>
        A contract forms when payment succeeds (or when a till sale is posted). Stock is reserved at add-to-cart for 15
        minutes. If payment does not complete, the reservation is released.
      </p>
      <p>
        You have 14 days from delivery or collection to withdraw, except sealed goods opened for hygiene where the seal
        is broken. See <a href="/legal/returns">Returns</a>.
      </p>
      <p>Ireland delivery and Dublin collection only at launch. We may refuse an order if stock cannot be fulfilled.</p>
      <p>Governing law: Ireland. Nothing here limits your statutory rights.</p>
    </article>
  );
}
